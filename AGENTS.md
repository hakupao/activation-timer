...
Codex and Codex CLIs at fixed times to keep usage windows "lit," then records activation
...
./install.sh check        # verify Codex/codex/jq/node/omc binaries
./install.sh dry-run      # print the exact Codex/codex command arrays, send nothing
...
bin/activate-ai-window.sh --once --tool Codex   # single real run, one tool (--tool all|Codex|codex)
```
...
- `bin/activate-ai-window.sh` — the activation runner (config load → lock → quota preflight →
  send prompt → record usage → post-run snapshot). Owns the Codex/Codex invocation arrays.
...
*sources*: Codex status depends on the **oh-my-Codex (`omc`) plugin** cache; Codex status
