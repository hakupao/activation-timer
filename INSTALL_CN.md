# Activation Timer 安装方式

Activation Timer 会发布成两种形态。

## 小白初学者：菜单栏 App

1. 下载 `activation-timer-gui-<version>.dmg`。
2. 打开 DMG，把 `Activation Timer.app` 拖到 `Applications`。
3. 打开 `Activation Timer.app`。
...
如果 macOS 提示 App 来自未识别开发者，右键点击 `Activation Timer.app`，
选择 **打开**，再确认一次。之后就可以正常双击打开。

## IT 高手：CLI / launchd

1. 下载 `activation-timer-cli-<version>.tar.gz`。
2. 解压。
